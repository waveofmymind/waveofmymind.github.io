...
특히 팀 프로젝트에서는 같은 백엔드라도 담당하는 영역이 다르다보니, 비즈니스 로직을 문서보다 코드로써 이해할 수 있는 것이 좋겠다고 생각했다.(그 팀원도 백엔드 개발자일 가능성이 높기 때문이다.)

그래서 내가 작성하는 코드가 이 프로젝트를 잊혀져갈 때 쯤의 내가 보았을 때 이해할 수 있고, 팀 프로젝트를 진행하며 타인에게 비즈니스 로직의 설명을 할 때 명확한 흐름이 보이는 코드를 작성하는 것에 대한 열망이 생겼다.  

즉, 미래에도 지속이 가능하고 확장에도 용이한, 지속 성장이 가능한 코드를 작성하는 것에 목표가 생긴 것이다.

그래서 이번 기회에 내 기존 코드를 리팩토링 하는 시간을 가졌다.

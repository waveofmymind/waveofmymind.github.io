...
이는 리팩토링할 책임을 미루는 것인가? 라는 생각이 들지만, 좋은 코드든 나쁜 코드든 기능이 확장될 필요가 있고, 성능 개선이 필요한 시점에선 무조건 필요하다.

그래서 지속적으로 나와 성장이 가능한 코드란, 언제 봐도 코드의 흐름을 파악할 수 있고 계층을 잘 분리한 코드라고 생각한다.
